module powerbi.extensibility.visual {
     ...
     * @property {number} yMax                 - Maximum data value in the set of data points.
     */
    ...
    function visualTransform(options: VisualUpdateOptions, host: IVisualHost): BarChartViewModel {
        ...
        console.log(datax1);
        this.target.innerHTML = "custom prop is " + this.datax1
        ...
        for (let i = 0, len = Math.max(category.values.length,datax1.values.length); i < len; i++) {
        ...
        yMax = <number>datay2.maxLocal;
        xMax = <number>datax2.maxLocal;
    ...
    export class BarChart implements IVisual {
        ...
        public update(options: VisualUpdateOptions) {
            ...
            let yScale = d3.scale.linear()
                .domain([0, viewModel.yMax])
                .range([height, 0]);

            let xScale = d3.scale.linear()
                .domain([0,viewModel.xMax])
                .range([0, width]);
        ...
        public valueForSource(dataView, sourceName: any) {
            let values = [];
        ...
        private calculateAverage(): number {
            ...
            this.barDataPoints.forEach((value: BarChartDataPoint) => {
                total += <number>value.x2-<number>value.x1;
            });
